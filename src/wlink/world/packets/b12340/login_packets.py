...
from wlink.utility.construct import Coordinates, GuidConstruct, PackEnum
...
class LoginFailureReason(Enum):
    ...
    locaked_by_billying = 7

SMSG_CHARACTER_LOGIN_FAILED = construct.Struct(
    'header' / ServerHeader(Opcode.SMSG_CHARACTER_LOGIN_FAILED, 1),
    'reason' / PackEnum(LoginFailureReason),
)

def make_SMSG_CHARACTER_LOGIN_FAILED(reason: LoginFailureReason):
    return SMSG_CHARACTER_LOGIN_FAILED.build(dict(
        header=dict(size=2 + 1),
        reason=reason
    ))
...
__all__ = [
    ...
    'SMSG_LOGIN_VERIFY_WORLD', 'make_SMSG_CHARACTER_LOGIN_FAILED', 'SMSG_CHARACTER_LOGIN_FAILED', 'LoginFailureReason'
]